...
## Sprint 18 - Eight characters, shown outright

### What was asked

Three things, from the owner: the invitation password 8 characters at most and alphanumeric;
no reveal step; and Share not gated behind a second auth check.

### What changed

- **Password length 20 → 8**, over the same unambiguous alphabet (57 characters, no `0/O/1/l/I`),
  which was already alphanumeric — only the length moved. Roughly 46 bits. Display grouping went
  from fives to fours so 8 characters read as two even halves. **Existing 20-character passwords
  keep working**: grouping is presentation only and `normalisePasswordInput` strips the hyphens
  before verification.
- **`toSummary` now decrypts `encryptedPassword`** and carries the plaintext on
  `InvitationSummary`. The listing route is admin-checked, so being the administrator is the
  whole of the authorization for reading a password they issued themselves. A gate in front of
  that only stood between the owner and their own data.
  `app/api/admin/invitations/[inviteId]/reveal-password/route.ts` is deleted.
- **`InvitationRow` shows the password outright.** Share builds its message from what is already
  on screen — no fetch, no reauthentication, because asking again would confirm nothing. The row's
  buttons are now Share / Copy password / Copy link / Rotate password / Enable-Disable.
- **A record that will not decrypt yields a summary with no password rather than throwing.** One
  invitation encrypted under a rotated or lost key must not take down the whole listing; the row
  reads "Unreadable — rotate it to issue a new one" and Share and Copy are disabled on it.
- **Rotation keeps `requireFreshAdmin`.** Destructive is a different question from readable: the
  old password stops working the instant the new one starts. This is now the only reauthentication
  path in the admin dashboard.
- **`password_revealed` stays in the `AdminActionType` union**, annotated as no longer written, so
  existing audit rows still render in `OverviewPanel`.

### The tradeoff, stated plainly

46 bits is far past what online guessing can reach — ten tries per fifteen minutes against 57^8 is
an expected hit in geological time — but it no longer has the margin to survive an **offline**
attack on a stolen hash, which 116 bits did. The scrypt work factor in `lib/security/password.ts`
is what carries that case now and **must not be lowered**. Worth holding alongside the existing
decision that `consumeAttempt` fails *open* when Firestore is unavailable: that was a comfortable
trade behind a 116-bit password and is a thinner one behind 46 bits, though still sound.

### Divergence from master_prompt.md

§25 still specifies reveal behind a Google reauthentication. The code departs from it deliberately,
at the owner's instruction. Unlike §38I in Sprint 17, **the spec was not amended** — the reasoning
lives in `lib/invitations/types.ts` and `README.md` instead. If `master_prompt.md` is ever treated
as authoritative again, this is the open reconciliation.

### Verification

`tsc --noEmit` clean, `eslint` clean (the one warning is pre-existing, in `scripts/tmp-compare.test.ts`),
228/228 vitest pass. The generation tests read `invitationGenerationParameters` rather than hard-coding
20, so the length change needed no test edit. No browser verification, per CLAUDE.md.

One stale artifact to know about: `.next/types/validator.ts` still referenced the deleted route and
failed `tsc` until it was removed. It is regenerated by the next build.

## Sprint 18 - Two-tier model routing, and a runaway field found live

**Status:** complete

### The tier, and the ceiling that would have made it pointless

`ModelEntry` gains `tier: "primary" | "reserve"`. `buildCandidates` walks tier-major:
every enabled primary on key 1, key 2, key 3 -- and only when all of those are spent, the
same walk over the reserve models. So a model that is merely rate-limited for a moment
falls to the next primary, never to the reserve; the reserve is reached only when the
strong models are genuinely exhausted across all three accounts.

**`MAX_ATTEMPTS` was 12.** A five-model configuration produces 15 candidates, and the
three it would have cut off are exactly the reserve tier -- the part that exists for the
day the strong models run out. Raised to 15, and `router.test.ts` now fails if a
configuration outgrows it, because nothing else would say so: an unreachable tail throws
nothing and fails no build.

`buildCandidates` and `MAX_ATTEMPTS` are exported for that test. The ordering *is* the
feature, and a regression in it is silent -- participants are quietly served a weaker
model while a stronger one was merely busy.

Stored configurations written before tiers have no `tier`; `normaliseTier` reads them as
primary. Dropping them would shrink the walk invisibly; reading them as reserve would
demote every model the administrator chose.

### What is actually configured

`gemini-3.6-flash` then `gemini-3.5-flash-lite`, both primary, then the next key.
**Nothing is in the reserve tier** (owner's decision), so the reserve pass is currently
empty and the walk is a single pass over 6 candidates. The mechanism stays live: one
click in the admin UI promotes a model.

`gemini-3.7-flash` is **not queried**. Every live attempt across all three accounts
returned `503 high demand` or nothing at all inside 90s. First in the walk, an
unavailable model costs each generation a full attempt timeout before anything working is
reached.

There is **no `gemini-3.1-flash`** in the live catalogue -- only `-flash-lite`,
`-pro-preview` and image/tts/live variants. A model id that does not exist would 404 on
every key on every call forever, because `classifyFailure` correctly reads 404 as "try the
next candidate" rather than as a configuration error.

### The admin panel shows the walk order, because otherwise it lies

`inWalkOrder()` sorts tier-major and renumbers. The panel's whole job is to state the
sequence, so a list showing a reserve above a primary would state a sequence the engine
does not use. Reordering is confined within a tier: a move across the boundary would look
like a promotion and change nothing.

`POST /api/admin/ai-config` now refuses a configuration with no enabled **primary** --
otherwise every participant is served the fallback as though it were the choice.

### `tension` ran away on three different models

Found by running the real prompt against 3.6-flash, 3.5-flash-lite and 3.1-flash-lite on
the same part-3 fixture. All three degenerated in the same field:

> "...perpetually indefinitely forward always indefinitely onwards forever continuously
> endlessly on and on without relief or rest anywhere in sight continuously forever..."

3.5-flash-lite hit `MAX_TOKENS` at 7,100 output tokens and truncated the JSON mid-string;
3.6-flash came back whole at 2,278 tokens and was rejected by zod as `too_big`. Either way
`parseJsonResponse` throws, the route answers 503, and the participant receives nothing --
and because validation happens *after* `generate()` returns, no fallback candidate is
tried. A degenerate field is a guaranteed failure, not a retryable one.

**The cause is the field, not the model.** `tension` is the one field instructed to state
both sides and offer no resolution, so nothing in the task tells the model it has finished.

Fixed in two places:

- `interpretationResponseSchema` now carries `maxLength` on every string, mirroring the
  zod caps. The prompt already said "tension 2 sentences" and all three models sailed past
  it -- stated only in the prompt it is advisory, in the schema the provider enforces it
  during decoding. This is the fix that binds.
- The prompt names the failure: this field resolves nothing, so it has no natural closing
  beat and is the one most likely to run on.

Re-verified live: 3.5-flash-lite went from 7,100 output tokens and a truncated response to
**199 tokens, 163 words, STOP**.

### maxOutputTokens 4096 -> 8192 for interpretation

`thinkingBudget` is a hint, not a cap: 3.1-flash-lite was measured spending 972 and 984
thinking tokens against a budget of 512. With thinking charged against the same budget, a
verbose response reached the 4096 ceiling and truncated. The ceiling is not a cost -- an
ordinary response uses ~400 output tokens -- and what it buys is that an over-long
reflection arrives instead of nothing (§75).

### Live measurements, this sprint

| Model | key1 | key2 | key3 |
| --- | --- | --- | --- |
| `gemini-3.6-flash` | 429 quota | 429 quota | 429 quota (hung earlier) |
| `gemini-3.5-flash-lite` | OK ~1.0s | OK ~1.0s | OK ~0.8s |
| `gemini-3.5-flash` | OK ~8.4s | OK ~10.2s | OK ~2.3s |
| `gemini-3.1-flash-lite` | OK ~4.7s | OK ~5.7s | OK ~4.8s |

**3.6-flash exhausted its free-tier daily quota on all three accounts within a single
session of testing**, while every other model stayed healthy on every key. Google no
longer publishes the free-tier table (the docs point at AI Studio, and limits are
per *project*, not per key -- three separate AI Studio accounts is what makes the key
pools multiply capacity at all). The practical reading: 3.6-flash has a small RPD and
should not be the only strong model in the walk.

The full interpretation prompt measures ~12,000 tokens; a completed exercise is 12 section
reflections plus 1 synthesis = **13 successful model calls**, ~168k input tokens.

### Verification

`npx tsc --noEmit`, `npx eslint`, `npx next build` clean. **228 tests passing** (up from
217), 11 of them new across candidate ordering, the attempt ceiling and tier normalisation.

**Not verified:** no fresh `gemini-3.6-flash` run after the schema fix -- its quota was
exhausted on all three keys by then. The fix is verified on 3.5-flash-lite, and the
`maxLength` bound is enforced by the provider rather than by anything model-specific.

---

## Handoff — start here

**Done: S0-S18.** The build plan is complete; S15 and S16 closed gaps found in live testing,
S17 rebuilt the knowledge base and exercise from the source book, S18 reworked model
routing and fixed a field that could fail a generation outright.
